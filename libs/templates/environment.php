...
abstract class EnvironmentTemplate extends Database {
	...
	public static function authUser($password, $username = null) {
		if ($username) { // A username password combination is provided
			...
			if (!static::passwordValid($password, $user['password'])) return;
			...
			if (empty($user['token']) || $user['expired'] == 1) {
				...
				// Delete old tokens
				$eva->query('DELETE FROM tokens WHERE userid = '.mysqli_real_escape_string(static::$db, $user['id']));

				// Insert new token
				$result = $eva->query('INSERT INTO tokens (token, created, userid)
								VALUES("'.mysqli_real_escape_string(static::$db, $token).'", "'.time().'", '.mysqli_real_escape_string(static::$db, $user['id']).')');
				if (!$result) return;
			...
			// Security check..
			if (strlen($password) != 32) return;

			// Get username
			$eva = Eva::getInstance();
			$result = $eva->query('SELECT username FROM users AS a, tokens AS b
									WHERE a.id = b.userid AND b.token = "'.mysqli_real_escape_string(static::$db, $password).'"');
	...
	public static function getUser($username) {
		...
		$result = $eva->query('SELECT *
								FROM users
								WHERE username = "'.mysqli_real_escape_string(static::$db, $username).'"');
		if ($eva->lastNumRows() == 1) {
			...
			$result = $eva->query('SELECT *
									FROM tokens
									WHERE userid = '.mysqli_real_escape_string(static::$db, $user['id']).' ORDER BY expired DESC');
		...
		$result = $eva->query('INSERT INTO users  (`lastname` ,
													...
													`link`
														...
														"'.mysqli_real_escape_string($user['form']).'",
														"'.mysqli_real_escape_string($user['teacherTag']).'",
														"'.mysqli_real_escape_string($user['accessLevel']).'",
														"'.mysqli_real_escape_string($user['password2change']).'",
														"'.mysqli_real_escape_string($user['link']).'"
													)');
	...
	public static function generateToken() {
		do {
			...
			$eva = Eva::getInstance();
			$eva->query('SELECT token FROM tokens WHERE token = "'.static::escapeString($randomString).'"');
	...
	public static function destroyToken($id) {
		$eva = Eva::getInstance();
		$query = $eva->query('UPDATE tokens SET expired = 1 WHERE userid = "'.static::escapeString($id).'"');
		return ($query == true);
	}
	...
	public static function updatePass($user, $password, $password2change = 0) {
		...
		if ($link != -1) {
			if(!static::updateExternalPass($user, $hashedPass, $password2change)) return false;
		}

		// Update internal account
		$eva = Eva::getInstance();
		$result = $eva->query("UPDATE users SET password2change = ".$password2change.", password = '".static::escapeString($hashedPass)."'
									WHERE id = '".static::escapeString($user['id'])."'");
	...
	public static function createUser($lastname, $firstname, $accessLevel) {
		// $accessLevel valid?
		if (intval($accessLevel) != $accessLevel || $accessLevel < 0 || $accessLevel > 4) return false;
		...
		$result = $eva->query('INSERT INTO users  (
														...
														`link`
															...
															"'.static::escapeString($lastname).'",
															"'.static::escapeString($firstname).'",
															LOWER("'.static::escapeString($username).'"),
															"'.static::escapeString(static::encryptPassword(Config::DEFAULT_PASS)).'",
															"",
															"",
															"'.intval($accessLevel).'",
															1,
