	...
	if ($data) {
		...
		foreach($schedule as $row)
		{
			...
			if ($row["datum"] != $date) {
				...
				if ($datesCurrent == $datesMax) break;
				$datesCurrent ++;
		...
		foreach($dates as $date)
		{
			 ...
			 * [2] Notes
			 */
			if (!$debug) echo '<tr><td style="width: 100%;" colspan="4"><p style="display:inline;margin-right: 0;">Verhinderte Lehrkräfte: </p>'.$eva->generateNote($user, true, $date).'</td></tr>'."\n";
			...
			if (count($ausfall[$date]) > 0) {
				...
				if (!$debug) echo '<tr><td class="emptyRow" style="width: 100%;" colspan="4"></td></tr>'."\n";
				if (!$debug) echo '<tr><th style="width: 10%;">Stunde</th><th colspan="2" style="width: 60%;">Unter- und Mittelstufe</th><th>Oberstufe</th></tr>'."\n";
				...
				{	
					...
					{	
						...
						if ($rowCount == 1) {
							...
							if (!$debug && $stundeNo % 2 != 0) echo '<tr><td class="emptyRow" colspan="4"></td></tr>'."\n";
							...
							if (isset($aufsicht[$date][$currentStunde]) && count($aufsicht[$date][$currentStunde]) > 0) {
								...
								{	
									if (!$debug) echo '<tr><td style="width: 100%;" colspan="4"><b>Aufsicht:</b> '.Common::escapeStrings($row["lehrerid"]).'</td></tr>'."\n";
								}
							}

							// Print row (with rowspan..)
							if (!$debug) echo '<tr><td class="firstCell" rowspan="'.(($rowspan == 0) ? 1 : $rowspan).'">'.Common::escapeStrings($stundeNo).'.</td><td class="grey">'.getAusfall($us[0]).'</td><td class="grey">'.getAusfall($us[1]).'</td><td>'.getAusfall($os).'</td></tr>'."\n";
						} else {
							$stundeNo = null;
							if (!$debug) echo '<tr><td class="grey">'.getAusfall($us[0]).'</td><td class="grey">'.getAusfall($us[1]).'</td><td>'.getAusfall($os).'</td></tr>'."\n";
						}
					}
				}
			}
