...
class Component extends ComponentTemplate {
	...
	function getDefaultView() {
		...
		if ($this->validSession($_SESSION)) {
			...
			// Admin?
			if ($_SESSION['accessLevel'] == 4) return 'manage';
		}
			...
			case "login":
				if ($_SERVER['REQUEST_METHOD'] == 'POST') {
					...
					if (!empty($username) && !empty($password)) {
						...
						if (!$user) Common::redirect('/index.php?component=desktop&view=login&errorType=auth');

						if (isset($user['teacherTag']) && empty($user['teacherTag']) && $user['accessLevel'] < 2)  Common::redirect('/index.php?component=desktop&view=login&errorType=config');
	...
	function needsUpdate(){
		return (empty($_SESSION['form']) && $_SESSION['accessLevel'] < 2) || (isset($_SESSION['password2change']) && $_SESSION['password2change'] == true);
	}
}