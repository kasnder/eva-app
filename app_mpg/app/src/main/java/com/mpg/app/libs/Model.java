...
public class Model extends HashMap<String, Object> implements Serializable {
	...
	public Model (HashMap<String, String> columnsMap, JSONObject jsonObject/*, String dateFormat*/) throws JSONException, ParseException {
		...
		while (iterator.hasNext()) {
			...
			switch (columnsMap.get(key)) {
				...
				case ("date"): {
					Date value = new SimpleDateFormat(_dateFormat).parse(jsonObject.getString(key)); // TODO New SimpleDate..
					this.put(key, value);
					break;
				}
				case ("int"): {
	...
	/**
	 * Indicates if key is empty
	 * @param key key
	 * @return key empty?
	 */
	public boolean empty (Object key) {
		return this.get(key) == null
				|| (super.get(key) instanceof String && ((String) super.get(key)).isEmpty());
	}
}