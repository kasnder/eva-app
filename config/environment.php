...
class Environment extends EnvironmentTemplate
{
    ...
    {
        if ( ! static::connect(Config::MOODLE_HOST, Config::MOODLE_USER, Config::MOODLE_PWD, Config::MOODLE_DB)) {
    ...
    public static function getUserFallback($username)
    {
        // Get user infos
        $query
                = "SELECT id, firstname, lastname, username, password, confirmed
					FROM mdl_user
					WHERE deleted = 0 AND
							confirmed = 1 AND
        ...
        if ($status) {
            ...
            $query
                    = "SELECT roleid FROM `mdl_role_assignments`
						WHERE  roleid = ".mysql_real_escape_string(Config::MOODLE_TEACHER_ROLE_ID)." AND
						userid = '".mysql_real_escape_string($moodleId)."'";
            $result = static::query($query);
    ...
    {
        $query
                = "SELECT value FROM `mdl_user_preferences`
    ...
    public static function moodleExtra($param, $moodleId)
    {
        // Get id of the $param
        $query
                 = "SELECT id FROM `mdl_user_info_field`
					WHERE shortname = '".mysql_real_escape_string($param)."'";
        $result  = static::query($query);
        ...
        $query
                = "SELECT data FROM `mdl_user_info_data`
					WHERE userid = '".mysql_real_escape_string($moodleId)."' AND
							fieldid = '".mysql_real_escape_string($paramId)."'";
        $result = static::query($query);
    ...
    public static function sync($user)
    {
        ...
        $query
            = "SELECT id, firstname, lastname, username, password FROM mdl_user
        ...
        switch (static::lastNumRows()) {
            case 1:
                ...
                if ( ! static::checkProperty('firstname', $user['firstname'], $rows['firstname'], $moodleId)) {
                    return false;
                }
                if ( ! static::checkProperty('lastname', $user['lastname'], $rows['lastname'], $moodleId)) {
                    return false;
                }
                if ( ! static::checkProperty('password', $user['password'], $rows['password'], $moodleId)) {
                    return false;
                }
                if ( ! static::checkProperty('password2change', $user['password2change'], $password2change, $moodleId)) {
                    return false;
                }
                ...
                if ( ! empty($tag) && ! static::checkProperty('teacherTag', $user['teacherTag'], $tag, $moodleId)) {
                    return false;
                }

                // Get new user info
                $result = $eva->query('SELECT * FROM users WHERE username = "'.mysql_real_escape_string($user['username']).'"');
                $user   = mysql_fetch_assoc($result);
    ...
    {
        if ($baseValue == $correctValue || $correctValue === null) {
            return true;
        }

        $eva = Eva::getInstance();
        $eva->query("UPDATE users SET ".mysql_real_escape_string($name)." = '".mysql_real_escape_string($correctValue)."' WHERE link = '"
            .mysql_real_escape_string($moodleId)."'");

        return (mysql_affected_rows(static::$db) == 1);
    }
    ...
    public static function updateExternalPass($user, $hashedPass, $password2change)
    {
        ...
        // Update password
        $result = static::query("UPDATE mdl_user SET password = '".mysql_real_escape_string($hashedPass)."'
										WHERE id = '".mysql_real_escape_string($moodleId)."'");
        if ( ! $result) {
            return false;
        }
        ...
        switch ($count) {
            case 0: // Flag hasn't been created
                $query = "INSERT INTO mdl_user_preferences (`userid`, `name`, `value`) VALUES (".intval($moodleId).", 'auth_forcepasswordchange', "
                    .intval($password2change).")";
                break;
            case 1: // Flag has been created
                $query     = "UPDATE mdl_user_preferences SET value = ".$password2change."
										WHERE name = 'auth_forcepasswordchange' AND userid = '".mysql_real_escape_string($moodleId)."'";
                $old_value = mysql_result($result, 0);
                break;
            default:
                return false;
        }
        ...
        if (mysql_affected_rows(static::$db) == 1 || (isset($old_value) && $old_value == intval($password2change))) {
            return true;
        } else {
            return false;
        };
