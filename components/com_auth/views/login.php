...
switch ($user['accessLevel']) {
    ...
    case 4: { // admin
        if (Common::getRequest('os') == 'nativeAndroid') { // Use real a hrefs for android
            $note['info'] .= '<p><a href="'.Common::escapeStrings(Common::getUrl('/index.php?component=desktop&username='
                    .urlencode($user['username'])), true).'">Auf zur Administration!</a></p>';
        } else {
            $note['info'] .= '<p><a href="#" onClick="window.open('."'".Common::escapeStrings(Common::getUrl('/index.php?component=desktop&username='
                    .urlencode($user['username'])), true)."','_blank','location=yes','closebuttoncaption=Return');"
                .'">Auf zur Administration!</a></p>';
        }
        break;
    }
}
...
if (( ! isset($note['klasse']) || empty($note['klasse'])) && ( ! isset($note['teacherTag']) || empty($note['teacherTag']))
    && $note['isTeacher'] < 2
) {
